...
def calculate_clinical_safety_metrics(df_clean):
    ...
    no_alert_cases = len(df_clean[(df_clean['alert_resp'] == 'NO ALERT NEEDED') & (df_clean['alert_resp'] == df_clean['alert_true']) ])
    non_inter_alert_cases = len(df_clean[(df_clean['alert_resp'] == 'NON-INTERRUPTIVE ALERT') & (df_clean['alert_resp'] == df_clean['alert_true'])])
    ...
    orgi_no_alert_classes = ["NO DOCUMENTED REACTIONS OR INTOLERANCES", "NO REACTIVITY TO PRESCRIBED DRUG'S INGREDIENTS OR EXCIPIENTS"]
    orig_no_alert_def = len(df_clean[df_clean['classification_descr'].isin(orgi_no_alert_classes)] )
    ...
    class_alert_orig = ["DIRECT ACTIVE INGREDIENT REACTIVITY", "DIRECT EXCIPIENT REACTIVITY", "CHEMICAL-BASED CROSS-REACTIVITY TO EXCIPIENTS", "DRUG CLASS CROSS-REACTIVITY WITHOUT DOCUMENTED TOLERANCE", "DRUG CLASS CROSS-REACTIVITY WITH DOCUMENTED TOLERANCE"]
    # Alert burden reduction
    original_alert_burden = len(df_clean[df_clean['classification_descr'].isin(class_alert_orig)])  # How many would have had alerts originally
    current_alert_burden = len(df_clean[(df_clean['alert_resp'].isin(['INTERRUPTIVE ALERT','NO ALERT NEEDED'])) & (df_clean['alert_resp'] == df_clean['alert_true'])])   # How many have alerts now

    print(f"no_alert_cases: {no_alert_cases} orig_no_alert_def: {orig_no_alert_def} non_inter_alert_cases: {non_inter_alert_cases} total_cases: {total_cases}")
    alert_reduction_percentage = (no_alert_cases - orig_no_alert_def + non_inter_alert_cases)/total_cases # The total number of no_alert_cases for trad. CDSS is 41
    alert_burden_reduction =   alert_reduction_percentage *100  

    no_alert_need_percentage = (no_alert_cases / total_cases)
    non_interruptive_alert_percentage = (non_inter_alert_cases/total_cases)
    interruptive_alert_percentage = (inter_alert_cases/total_cases)
    return {
        # Basic distribution (keep as requested)
        'no_alert_cases': no_alert_cases,
        'no_alert_needed_percentage': no_alert_need_percentage,
...
def create_false_negative_analysis_plot(df_clean, output_file):
    ...
    # HELIOT behavior
    heliot_no_alert = len(df_clean[(df_clean['alert_resp'] == 'NO ALERT NEEDED') & (df_clean['alert_resp'] == df_clean['alert_true'])])
    heliot_no_interruptive = len(df_clean[(df_clean['alert_resp'] == 'NON-INTERRUPTIVE ALERT') & (df_clean['alert_resp'] == df_clean['alert_true'])])
    heliot_alert = total_cases - heliot_no_alert - heliot_no_interruptive
    ...
    categories = ['Alerts Generated', 'No Alert Cases', 'Non-Interruptive Alert']
    traditional_counts = [traditional_alert, traditional_no_alert, 0]
    heliot_counts = [heliot_alert, heliot_no_alert, heliot_no_interruptive]
    ...
    orgi_no_alert_classes = ["NO DOCUMENTED REACTIONS OR INTOLERANCES", "NO REACTIVITY TO PRESCRIBED DRUG'S INGREDIENTS OR EXCIPIENTS"]
    orig_no_alert_def = len(df_clean[df_clean['classification_descr'].isin(orgi_no_alert_classes)] )
    reduction_pct = ((heliot_no_alert - orig_no_alert_def + heliot_no_interruptive)/total_cases)*100 if total_cases > 0 else 0
...
def main(result_file, iter, suffix):
    print("Loading results...")
    print(f'./results/{iter}/{result_file}')
    try:
        df = pd.read_excel(f'./results/{iter}/{result_file}', dtype={'drug_code': str, 'leaflet': str, 'patient_id': str, 'classification':str})  
    ...
    with open(report_filename, 'a') as f:
        ...
        f.write(f"Non-interruptive Alert: {clinical_metrics['non_inter_alert_cases']} ({clinical_metrics['non_interruptive_alert_percentage']*100:.1f}%)\n")
        f.write(f"Interruptive Alert: {clinical_metrics['inter_alert_cases']} ({clinical_metrics['interruptive_alert_percentage']*100:.1f}%)\n\n")
    ...
    #df_clean.to_excel(f'./results/{iter}/results_full_synth_it.xlsx', index=False)
if __name__ == "__main__":
    # Set up the argument parser
    parser = argparse.ArgumentParser(description="Analyze the file.")
    
    # Add arguments for the files
    parser.add_argument("--input", help="Path to the file input path.", default="it")
    parser.add_argument("--suffix", help="Suffix for the output files.", default="it")
    parser.add_argument("--result_file", help="Result file name.", default="results_full_synth.xlsx")
    # Parse the arguments
    args = parser.parse_args()

    main(args.result_file, args.input, args.suffix)
